...
$(document).ready(function() {
    ...
    $('#addItems').on('click', function() {
        var inputData = $('#inputData').val().trim();
        if (inputData) {
            ...
            lines.forEach(function(line) {
                ...
                if (parts.length === 2) {
                    ...
                    var count = parseInt(parts[1].trim(), 10);
                    if (nickname && !isNaN(count)) {
                        addItem(nickname, count);
                    }
                }
            });
            updateData();
    ...
    function addItem(nickname, count) {
        var $li = $('li.item.empty').first().clone().removeClass('empty');
        $li.find('.item-title').text(nickname);
        $li.find('.item-weight').attr('data-weight', count);
        $('ul').append($li);
    }
});
...
function updateData() {
	...
	$('li.item:not(.empty)').each(function(i, e) {
		...
		$e.css('background', 'linear-gradient(to right, ' + color + ' 11%, rgba(0, 0, 0, 0) 11%)');
	});
